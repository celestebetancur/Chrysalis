...
# For Windows: use Chrysalis-owned pre-generated parser (win-parser/chuck_yacc.c)
# For Linux/Mac: use the submodule's chuck_yacc.c (regenerated by bison/flex below)
ifdef ARCH_WIN
SOURCES += win-parser/chuck_yacc.c
FLAGS += -iquote win-parser
else
SOURCES += $(CHUCK_SRC_DIR)/chuck_yacc.c
endif
...
# - win-parser/chuck_yacc.c and win-parser/chuck.tab.h are committed to Chrysalis
#   and generated with the same bison/flex version, so they are guaranteed to match.
# - The dep target copies win-parser/chuck.tab.h into chuck/src/core/ so that
#   other chuck headers that include chuck.tab.h resolve correctly.
dep:
	cp win-parser/chuck.tab.h $(CHUCK_SRC_DIR)/chuck.tab.h

clean_generated:
	rm -f $(CHUCK_SRC_DIR)/chuck.tab.h
endif
